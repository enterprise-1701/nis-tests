...
public class NWAPIV2_CustomerCredentialsAuthenticate_InvalidPassword_MSD extends NWAPIV2_CustomerCredentialsAuthenticate_InvalidPassword {
	...
	protected void verifyResponse(Hashtable<String,String> data, RESTActions restActions, String response) {
		...
		// MSD only
		String expectedErrorMessage = data.get(EXPECTED_ERROR_MESSAGE);
		String actualErrorMessage = errorMsg.getAuthErrors().get(0).getErrorMessage();	
		restActions.assertTrue(expectedErrorMessage.equals(actualErrorMessage), 
				String.format(BAD_ERROR_MESSAGE_FMT,expectedErrorMessage,actualErrorMessage));		
	}
}
